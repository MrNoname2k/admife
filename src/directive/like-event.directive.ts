    ...
    if (event) {
      if (this.data) {
        const listLiked = this.data.likes.map((like) => {
          if (like.delFlg === 0) {
            return like.userEntityLike.lastName + ' ' + like.userEntityLike.firstName;
          }
          return null;
        });
        ...
        this.likeService.creatLike(this.data).subscribe((res) => {
          ...
          if (res.data.delFlg === 0) {
            if (!listLiked.includes(res.data.userEntityLike.firstName && res.data.userEntityLike.lastName)) {
              listLiked.push(res.data.userEntityLike.lastName + ' ' + res.data.userEntityLike.firstName);
            }
            ...
            const likeUsersAfterUnlike = listLiked.filter(
              (element) => element !== res.data.userEntityLike.firstName && element !== res.data.userEntityLike.lastName
            );
